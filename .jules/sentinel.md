...
## 2024-05-20 - [Fix Denial of Service (DoS) risk via URIError in dynamic routes]
**Vulnerability:** Unhandled URIError in Next.js dynamic routes caused by calling `decodeURIComponent` on untrusted route parameters (`params.slug`). If an attacker provides an invalid URL-encoded string (e.g., `%2`), `decodeURIComponent` throws an exception, leading to a 500 Internal Server Error crash (Application-level DoS).
**Learning:** In Next.js App Router, dynamic route parameters must always be treated as untrusted user input, even before they reach application logic. Standard library functions that parse or decode strings (like `decodeURIComponent` or `JSON.parse`) can throw unhandled exceptions if the input is malformed, bypassing typical validation steps.
**Prevention:** Always wrap standard parsing/decoding functions in `try...catch` blocks when operating on user-provided input, including route parameters, query strings, and request bodies. Fail securely by returning a user-friendly error state or a 400 Bad Request response instead of allowing the application to crash.
