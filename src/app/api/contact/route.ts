...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      // SECURITY: Explicit feature flag for Ethereal test account to prevent accidental PII leakage
      // in environments where NODE_ENV is not exactly 'production'.
      if (process.env.ENABLE_TEST_EMAIL === 'true') {
        console.warn('Configurazione email mancante. Utilizzo account di test Ethereal abilitato tramite ENABLE_TEST_EMAIL.');
